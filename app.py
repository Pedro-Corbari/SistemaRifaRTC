...
app = Flask(__name__, static_folder='C:/Users/pedro/Desktop/projetosPython/rifaAraucCopa/static')


@app.route("/", methods=["GET", "POST"])
def homepage():
    variavel = "Insira o número:"
...
def form():
    if request.method == "GET":
        ...
        if numero_rifa_dados == '' or comprador =='' or Vendedor=='':
            return render_template('numero_proib.html')
        numero_rifa_dados = int(numero_rifa_dados)
        if numero_rifa_dados <= 0 or numero_rifa_dados > 200:
            return render_template('numero_proib.html')

        consulta_banco.insere_dados(int(numero_rifa_dados), comprador, telefone, Vendedor)
